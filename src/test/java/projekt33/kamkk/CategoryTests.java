...
public class CategoryTests {
    ...
    void unsuccessfulDeleteByIdAndThemesIsNotNull() {
        themeRepository.save(Theme.builder().category(categoryRepository.findById(categoryId).orElseThrow(EntityNotFoundException::new)).createdAt(Calendar.getInstance().getTime()).title("Theme Test").build());
        categoryRepository.deleteByIdAndThemesIsNull(categoryId);
        List<Category> categories = categoryRepository.findAll();
        assertEquals(1, categories.size());
    }
